...
```{r}
table = xtabs(~ Severity + Amenity+Bump+Crossing+Give_Way+Junction+No_Exit+Railway+Roundabout+Station+Stop+Traffic_Calming+Traffic_Signal, data = df)
table
```
Interpretation - Two-way contingency table

From the preliminary results generated by the two-way contingency table, we can see that, when there was an amenity, and no other predictor variables in place, there was 3459 accidents that had a severity level of "2", 415 accidents that had a severity level of "4", 330 accidents that had a severity level of "3", and 77 accidents that had a severity level of "1"

We can also see that all of the accidents happened when the following road conditions were present: "Crossing", "Give-Way", "Junction", and "Amenity".

This result, although preliminary, showed us that these road conditions, especially "Amenity", affected the severity of the accident, however, further analysis is needed. 
