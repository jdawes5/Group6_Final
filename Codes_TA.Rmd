...
newdata <- subset(dataset, select = c(Severity, Temperature.F., Wind_Speed.mph.))

#scale vars
scaleddata <- as.data.frame(scale(newdata[2:3], center = TRUE, scale = TRUE))
scaleddata <- cbind(newdata[,1], scaleddata)
names(scaleddata)[1] <- 'Severity'

#dealing with class imbalance by generating stratified random split of data
set.seed(123)
trainIndex <- createDataPartition(scaleddata$Severity, p = .7, list = FALSE, times = 1)
carter_train <- scaleddata[trainIndex,]
carter_test <- scaleddata[-trainIndex,]

knn_pred <- knn(train = carter_train[,2:3], test = carter_test[,2:3], cl=carter_train$Severity, k=7)
cross_table <- CrossTable(carter_test$Severity, knn_pred, prop.chisq = FALSE) #analyze these results later
```

#What factors best predict the severity of an accident?

```{r}
#use RF model GINI index and VarImp plots here
```
